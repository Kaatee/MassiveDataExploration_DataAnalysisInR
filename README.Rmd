...
```
***[TODO]*** może to jakoś ładniej zrobić jakimś forem "as.numeric" a nie tak brzydko powtarzając kod
...
```
***[TODO]*** może też jakiś for , aczkolwiek Bartek tak ma więc może to nie jest aż takie brzydkie
...
***[TODO]*** Opisać reszte  
***[TODO]*** Moze cos z odstającymi danymi, ogarnąć czy jest sens
...
***[TODO]*** Zmniejszyć tekst w srodku zeby bylo widac  jeśli się da, jeśli dłużej niż 10min to olać :P
...
inTraining <- createDataPartition(y = data2$length, p = .75,list = FALSE)
...
```{r cache=TRUE}
fitRandomForestF <- train(length ~ ., data = training, method = "rf", importance=T, trControl = ctrl, ntree = 10) #rf
fitRandomForestF
...
***[TODO]*** zrobić żeby był ładniej ten wykres a nietakie powtarzane te parametry  
...
Na koniec modele zostały porównane na podstawie miary RMSE (w przypadku tej miary, im mniejszą wartość osiągnie algorytm, tym jest lepszy) i R^2:  
***[TODO]*** Poczytać o R^2 i napisac cos o tym czy lepsze jak jest wieksze czy jak  
***[TODO]*** Zrobic zeby  R^2 ładnie sie pisało z ym 2 na górze
...
***[TODO]*** TODO - jak już puścimy i sie nauczy to zobaczyc które z tych ogarnąć i opisać
